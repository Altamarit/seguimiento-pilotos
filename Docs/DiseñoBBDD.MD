# Diseño BBDD - Supabase

## Objetivo

Definir la estructura SQL mínima para el MVP en Supabase, alineada con el `PRD.md` y `CasosDeUso.md`.

Decisiones clave del modelo:

- El total de `personas formadas` se guarda manualmente en la tabla de pilotos.
- Los eventos de tipo `formacion` guardan cuántas personas se formaron en ese evento, pero no recalculan automáticamente el total del piloto.
- La `mejora de productividad` se obtiene a partir del último evento de tipo `productividad` de cada piloto.
- Los roles de aplicación son `lector`, `editor` y `admin`.

## Tablas incluidas

- `profiles`: perfil básico del usuario autenticado.
- `user_roles`: rol actual del usuario.
- `pilots`: pilotos de IA.
- `impact_events`: eventos de impacto asociados a un piloto.
...
- En la UI conviene mapear los labels visibles a enums internos:
  - `planificado` -> `planificado`
  - `en marcha` -> `en_marcha`
  - `finalizado` -> `finalizado`
  - `cancelado` -> `cancelado`
- El campo `trained_people_count` pertenece al piloto y debe editarse manualmente desde su detalle.
- El campo `trained_people_event` solo documenta el impacto puntual de un evento de formación.
- Si más adelante quieres auditar cambios, se puede añadir una tabla `audit_log` sin romper este diseño.
